...
public static class RankedSeedMath
{
...
function Get-ActiveSeedPath {
    ...
    return Join-Path $ResolvedGameRoot "SubnauticaSpeedrunningRanked\Data\Seeds\active-seed.xml"
}
...
function Resolve-SeedDefinition {
    ...
    $survival = $SeedXml.RankedSeedDefinition.Survival

    $result = [ordered]@{}
    $result.SeedId = $ResolvedSeedId
    $result.SeedValue = $ResolvedSeedValue
    $result.Description = [string]$SeedXml.RankedSeedDefinition.Description
    ...
    if ($ranges.Count -gt 0) {
        ...
        $selectedIndex = [RankedSeedMath]::SelectWeightedIndex($rootKey, "survival-spawn-range", $weights)
        ...
        $spawnX = [RankedSeedMath]::NextFloat($rootKey, "survival-spawn-x|$rangeName", (Get-DoubleOrDefault $selectedRange.MinX), (Get-DoubleOrDefault $selectedRange.MaxX))
        $spawnZ = [RankedSeedMath]::NextFloat($rootKey, "survival-spawn-z|$rangeName", (Get-DoubleOrDefault $selectedRange.MinZ), (Get-DoubleOrDefault $selectedRange.MaxZ))
    ...
    foreach ($groupName in $groups.Keys) {
        ...
        foreach ($entry in $groups[$groupName]) {
            ...
            if ($useSeedRange) {
                ...
                $resolvedValue = [RankedSeedMath]::NextSteppedFloat($rootKey, "survival|$groupName|$name", $minValue, $maxValue, $stepValue)
                $seededRollCount++
    ...
    foreach ($entry in @($survival.ManualCreatureSpawns.Entry)) {
        ...
        if ($useSeedRange) {
            ...
            $resolvedAmount = [RankedSeedMath]::NextSteppedInt($rootKey, "survival|ManualCreatureSpawns|$techTypeName|Amount", $minAmount, $maxAmount, $amountStep)
        }
        else {
...
$fileSeedId = [string]$seedXml.RankedSeedDefinition.SeedId
$fileSeedValue = [string]$seedXml.RankedSeedDefinition.SeedValue
