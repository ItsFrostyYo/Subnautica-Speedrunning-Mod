...
{
    internal sealed class ModSeedLifepodPlacementRuntime
    {
        ...
        public ModSeedLifepodPlacementRuntime()
        {
        ...
        public void Schedule(float targetX, float targetZ, string description)
        {
            ...
            ModLog.Info("Scheduled seeded lifepod spawn placement for target " + targetKey + ": " + _description + ".");
        }
        ...
        public void Update(bool canApplyPlacement)
        {
            ...
            if (TryApply())
            {
                if (!_appliedForCurrentSchedule)
                {
                    ...
                    if (!string.Equals(targetKey, _lastAppliedTargetKey, StringComparison.Ordinal))
                    {
                        ...
                        ModLog.Info("Applied seeded lifepod spawn: " + _description + ".");
                    }
                }
                ...
                {
                    ModLog.Info("Confirmed seeded lifepod spawn at target " + BuildTargetKey(_targetX, _targetZ) + ".");
                    _pending = false;
                }
            ...
            {
                ModLog.Warn("Timed out waiting for lifepod instance while applying seeded lifepod spawn: " + _description + ".");
                Reset();
            }
        }
