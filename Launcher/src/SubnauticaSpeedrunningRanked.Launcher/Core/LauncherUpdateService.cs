...
internal static class LauncherUpdateService
{
    ...
    public static bool TryRunUpdateFlow(FileLayout layout, string[] args)
    {
        ...
        bool accepted = LauncherAlert.Confirm(
            "Update Available",
            "A new Subnautica Speedrunning Ranked client update is available." + Environment.NewLine + Environment.NewLine
        ...
        string updaterSourceRoot = Path.Combine(layout.RankedRoot, "Updater");
        if (!Directory.Exists(updaterSourceRoot))
        {
            ...
            LauncherAlert.ShowError(
                "Updater Missing",
                "The updater files are missing from the ranked install. Reinstall the ranked client package first.");
            return false;
        }
        ...
        string updaterExecutablePath = Path.Combine(tempUpdaterRoot, "Ranked Updater.exe");
        if (!File.Exists(updaterExecutablePath))
        {
            ...
            LauncherAlert.ShowError(
                "Updater Missing",
                "The updater executable is missing from the ranked install. Reinstall the ranked client package first.");
            return false;
        }
        ...
        startInfo.ArgumentList.Add(@"SubnauticaSpeedrunningRanked\Launch Ranked.exe");
